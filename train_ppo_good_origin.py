...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        while not terminal:
            prediction = actor(state.to(device))
            ...
            if len(replay_memory) > opt.batch_size:
                ...
                with torch.no_grad():
                    ...
                    advantage = compute_advantage(opt.gamma, opt.lmbda, td_delta.cpu())
                ...
                for _ in range(opt.epochs):
                    for index in BatchSampler(SubsetRandomSampler(range(opt.batch_size)), opt.mini_batch_size, False):
                        ...
                        critic_loss = torch.mean(nn.functional.mse_loss(critic(states[index]), td_target[index].detach()))
