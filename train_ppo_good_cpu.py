...
def train(opt):
    ...
    while iter < opt.num_iters:
        ...
        if (iter + 1) % 1000 == 0:
            ...
            actor_saved_path = os.path.join(opt.saved_path, f"flappy_bird_cpu_actor_{iter + 1}")
            critic_saved_path = os.path.join(opt.saved_path, f"flappy_bird_cpu_critic_{iter + 1}")
