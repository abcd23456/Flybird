...
def train(opt):
    ...
    if os.path.exists(f"{opt.saved_path}/flappy_bird_actor"):
        checkpoint = torch.load(f"{opt.saved_path}/flappy_bird_actor", map_location=device)
        actor.load_state_dict(checkpoint['net'])
        actor_optimizer.load_state_dict(checkpoint['optimizer'])
        print("load actor succ")

    if os.path.exists(f"{opt.saved_path}/flappy_bird_critic"):
        checkpoint = torch.load(f"{opt.saved_path}/flappy_bird_critic", map_location=device)
    ...
    while iter < opt.num_iters:
        ...
        if (iter + 1) % 1000 == 0:
            ...
            torch.save(actor_dict, f"{opt.saved_path}/flappy_bird_actor")
            torch.save(critic_dict, f"{opt.saved_path}/flappy_bird_critic")
